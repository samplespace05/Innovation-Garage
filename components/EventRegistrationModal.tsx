...
export default function EventRegistrationModal({ isOpen, onClose, event }: ModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 2. Extract Custom Answers in Order (Safety check added)
    const customAnswers = event.config 
        ? event.config.map(q => formData[q.label] || "") 
        : [];

    try {
      // 🔴 UPDATED: Point to the Universal Gateway
      const res = await fetch("/api/admin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: "registerEvent", // 🟢 CRITICAL: Tells server to use the Registration Logic
          eventID: event.id,
          eventName: event.title,
          common,
          customAnswers
          ...
          >
            ...
            {status === "success" ? (
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-6">
                ...
                {/* DYNAMIC FIELDS (From Admin Config) */}
                {event.config && event.config.length > 0 && (
                  <div className="border-t border-gray-800 pt-6 mt-6">
                    <h4 className="text-neon-orange font-pixel text-sm uppercase mb-4">Additional Details</h4>
                    <div className="space-y-4">
                      {event.config.map((q) => (
                        <div key={q.id}>
                          ...
                          {q.type === "dropdown" ? (
                            ...
                            >
                              <option value="">Select Option</option>
                              {/* If you add options logic later, map them here */}
                            </select>
                          ) : (
                            <input 
                              type={q.type} 
                ...
                {status === "error" && (
                    <div className="bg-red-900/20 border border-red-500 p-3 text-center">
                        <p className="text-red-500 text-sm font-mono">⚠️ System Error: Connection Refused.</p>
                        <p className="text-red-400 text-xs">Please check your internet or try again later.</p>
                    </div>
                )}
              </form>
            )}
